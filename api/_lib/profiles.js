...
export function profilePatch(input) {
  ...
  if ("instagram" in input) {
    try {
      patch.instagram = instagramUsername(input.instagram);
    } catch (error) {
  ...
  for (const [key, limit] of Object.entries(fields)) {
    ...
    if (typeof input[key] !== "string" || input[key].trim().length > limit)
      throw Object.assign(new Error(`Isian ${key} tidak valid.`), {
        status: 400,
      });
  ...
  if (patch.photo && !patch.photo.startsWith("/assets/images/")) {
    try {
      if (new URL(patch.photo).protocol !== "https:") throw new Error();
    } catch {
      throw Object.assign(
        new Error("Foto harus menggunakan URL HTTPS atau aset website."),
        { status: 400 },
      );
    }
  }
...
export function rejectFields(input, allowed) {
  if (Object.keys(input).some((key) => !allowed.includes(key)))
    throw Object.assign(new Error("Ada kolom yang tidak diizinkan."), {
      status: 400,
    });
}
