...
import {
  ...
  showToast,
  uid,
  normalizeClassPhotoUrl,
  safeClassPhotoUrl,
  CLASS_PHOTO_PLACEHOLDER,
} from "../utils.js";
...
function render() {
  if (panel.querySelector(".is-button-loading")) {
    deferredRender = true;
    return;
  }
  if (
  ...
  <label for="admin-field-10">Foto kelas</label>

  <input
    name="classPhoto"
    value="${escapeHTML(profile.classPhoto || "")}"
    placeholder="Tempel link Google Drive atau URL gambar..."
    id="admin-field-10"
  >

  <p class="form-help" id="classPhotoHelp">
    Mendukung link Google Drive, direct image URL HTTPS, dan /assets/images/....
  </p>

  <div class="class-photo-admin-preview">
    <img
      id="classPhotoPreview"
      src="${escapeHTML(safeClassPhotoUrl(profile.classPhoto))}"
  ...
  if (active === "users" && !classMode)
    panel.innerHTML = hasRole(session.role, "developer")
      ? `<div class="admin-section"><div class="card form-card"><p class="eyebrow">developer only</p><h2>Buat Akun</h2><p class="muted">Buat akun untuk anggota kelas. Pengguna dapat masuk memakai username atau NIS.</p><form id="userForm"><div class="form-grid" style="margin-top:18px"><div class="form-field">
  <label for="admin-field-19">Nama lengkap</label>
  <input
    name="name"
    required
    id="admin-field-19"
...
<div class="form-field">
  <label for="admin-field-nickname">Nama panggilan</label>
  <input
    name="nickname"
    maxlength="40"
    placeholder="Contoh: Javier"
    id="admin-field-nickname"
  >
  <p class="form-help">
    Digunakan untuk sapaan di beranda.
  </p>
</div>

<div class="form-field">
  <label for="admin-field-20">Username / NIS</label>
  <input
    name="username"
    required
    id="admin-field-20"
  ...
  if (active === "tasks") {
    ...
    form.elements.due
      ...
      .insertAdjacentHTML(
        "afterend",
        '<div class="form-field"><label for="taskDueTime">Jam tenggat (opsional)</label><input id="taskDueTime" name="due_time" type="time"><p class="form-help">Kosongkan jika tugas berakhir pukul 23:59 WIB.</p></div>',
      );
...
panel.addEventListener("input", (e) => {
  if (e.target.name !== "classPhoto") return;

  const preview = panel.querySelector("#classPhotoPreview");
  const help = panel.querySelector("#classPhotoHelp");

  if (!preview || !help) return;

  try {
    const normalized = normalizeClassPhotoUrl(e.target.value);

    preview.onerror = () => {
      preview.onerror = null;
      preview.src = CLASS_PHOTO_PLACEHOLDER;

      help.textContent =
        "Gambar gagal dimuat. Pastikan file dapat diakses publik.";
    };

    preview.src = normalized;

    if (normalized.includes("drive.google.com/thumbnail?")) {
      help.textContent =
        "✓ Link Google Drive terdeteksi dan akan dikonversi otomatis.";
    } else {
      help.textContent = "✓ Link foto valid.";
    }
  } catch (error) {
    preview.onerror = null;
    preview.src = CLASS_PHOTO_PLACEHOLDER;
  ...
  try {
    if (e.target.getAttribute("id") === "profileForm") {
      ...
      try {
        ...
        status.hidden = false;
        form.querySelector('[name="classPhoto"]')?.focus();
        return;
      }
      ...
      if (name) {
        ...
        queue = [
          ...queue,
          {
            id: uid("apel"),
            name,
            memberId: matches.length === 1 ? matches[0].id : "",
          },
        ];
        render();
      }
      ...
      await withButtonLoading(
        ...
        async () => {
          await saveTask({
            ...
            due: f.due,
            due_at: f.due_time
              ? new Date(`${f.due}T${f.due_time}:00`).toISOString()
              : null,
            teacher: f.teacher,
...
panel.addEventListener("click", async (e) => {
  ...
  if (t.dataset.editTask) {
    ...
    if (form.elements.due_time) {
      ...
      form.elements.due_time.value = date
