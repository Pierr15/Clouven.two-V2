...
titleWriter.setText(
  ...
  titleElement.querySelector("em")?.textContent || "",
);
...
document
  ...
  .classList.toggle("is-signed-in", signedIn);
const key = todayKey();
let profile,
...
function render() {
  const profilePending = showLoadState(
    document.querySelector("#classFacts"),
    states.profile,
  );
  if (profile && !profilePending) {
    ...
    photo.onerror = () => {
      photo.onerror = null;
      photo.src = CLASS_PHOTO_PLACEHOLDER;
    };

    photo.src = safeClassPhotoUrl(profile.classPhoto);
    photo.alt = `Foto kelas ${profile.className}`;
    document.querySelector("#photoCaption").textContent =
      profile.photoCaption || profile.className;
  ...
  document.querySelector("#todayText").textContent =
    `Hari ini, ${formatLongDate()}.`;
  const hour = new Date().getHours();
  const greeting =
    hour < 11 ? "pagi" : hour < 15 ? "siang" : hour < 18 ? "sore" : "malam";
  const greetingName = session.profile?.nickname?.trim() || "teman";

  document.querySelector("#todayGreeting").innerHTML =
    `Selamat ${greeting}, <span>${escapeHTML(greetingName)}</span>`;
  const activeTasks = tasks
    .filter((task) => !progress[task.id]?.done)
    .sort((a, b) => {
      const now = Date.now(),
        ad = deadlineInstant(a.due_at || a.due)?.getTime(),
        bd = deadlineInstant(b.due_at || b.due)?.getTime();
      const af = ad >= now,
        bf = bd >= now;
  ...
  const todaysEvents = events.filter(
    (event) => new Date(event.start_at) < end && new Date(event.end_at) > start,
  );
...
if (can(session.role, "task_summary")) {
  subscribeTaskSummaries((v) => {
    tasks = v.filter((t) => t.status !== "archived");
  }, states.tasks.update);
  subscribeProgress(session.user.id, (v) => {
    progress = v;
    render();
  });
}
if (signedIn) {
  ...
  const loadEvents = async () => {
    states.events.update({
      status: "loading",
      hasData: Boolean(events.length),
    });
    try {
      events = await listCalendarEvents(start.toISOString(), end.toISOString());
      states.events.update({
        status: "ready",
        hasData: true,
        retry: loadEvents,
...
setTimeout(() => location.reload(), nextDay - Date.now());
