...
export const CLASS_PHOTO_PLACEHOLDER = "/assets/images/class-placeholder.svg";

export function normalizeClassPhotoUrl(value = "") {
  const raw = String(value).trim();

  if (!raw) return CLASS_PHOTO_PLACEHOLDER;
  ...
  try {
    ...
    throw new Error(
      "Foto kelas harus berupa link HTTPS, link Google Drive, atau aset /assets/images/.",
    );
  }

  if (url.protocol !== "https:") {
    throw new Error("Link foto kelas harus menggunakan HTTPS.");
  }

  const hostname = url.hostname.replace(/^www\./, "").toLowerCase();

  // Instagram post bukan direct image
  if (hostname === "instagram.com" || hostname.endsWith(".instagram.com")) {
    throw new Error(
      "Link posting Instagram tidak dapat digunakan sebagai foto kelas. Gunakan Google Drive atau direct image URL.",
    );
  }
  ...
  if (hostname === "drive.google.com") {
    ...
    if (!fileId) {
      throw new Error(
        "File ID Google Drive tidak ditemukan pada link tersebut.",
      );
    }
  ...
  // Direct URL eksternal
  return url.href;
}

export function safeClassPhotoUrl(value = "") {
  try {
    return normalizeClassPhotoUrl(value);
  } catch {
    return CLASS_PHOTO_PLACEHOLDER;
  }
}
...
export function todayKey(date = new Date()) {
  return DAYS.find((day) => day.jsDay === date.getDay())?.key || null;
}
export function dayInfo(key) {
...
export function taskState(task, done = false) {
  ...
  if (deadline && deadline < new Date())
    return { label: "Terlambat", cls: "is-late" };
  return { label: "Berjalan", cls: "" };
}
export function subjectColor(subject = "") {
  const sum = [...String(subject)].reduce(
...
export function openModal(wrap, trigger) {
  ...
  function close() {
    ...
    if (trigger?.isConnected) trigger.focus();
      ...
      (
        document.querySelector('#taskForm input[name="title"]') ||
        document.querySelector(".member-card, .app-main button")
      )?.focus();
  }
